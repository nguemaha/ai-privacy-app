...
def deidentify_data(
    df: pd.DataFrame, k: int, salt: str, cols_to_check: list[str]
) -> tuple[pd.DataFrame, int]:
    df_clean = df.copy()

    # A. Masking Direct Identifiers (Hashing)
    def hash_val(val: object) -> str:
...
"""
    ...
    k_value = st.sidebar.slider("Select K-Anonymity Level (k)", 1, 5, 1)
    hash_salt = st.sidebar.text_input(
        "Encryption Salt", "HMA-2026-SECURE", type="password"
    )
...
"""
    ...
    processed_df, suppressed_count = deidentify_data(
        raw_df, k_value, hash_salt, selected_quasi_identifiers
    )

    # --- 3. UI TABS ---
    tab1, tab2, tab3 = st.tabs(
        ["📊 Data Transformation", "📈 Utility Analysis", "📜 Governance Report"]
    )
    ...
    with tab1:
        ...
        with col1:
            ...
            st.error(
                "⚠️ Risk: PII exposure, HIPAA non-compliance. [Learn more about HIPAA compliance](https://www.hhs.gov/hipaa/for-professionals/privacy/index.html)"
            )
        ...
        with col2:
            ...
            if suppressed_count > 0:
                st.warning(
                    f"ℹ️ {suppressed_count} rows suppressed to maintain K-Anonymity."
                )
            else:
    ...
    with tab2:
        ...
        st.info(
            "Notice: Despite de-identification, the high-level clinical trends remain visible for research."
        )
    ...
    **Transformations Applied:**
    ...
    st.divider()
    st.caption(
        "Developed by Healthy Moms Action (HMA) - AI Security & Governance Portfolio Case Study."
    )
