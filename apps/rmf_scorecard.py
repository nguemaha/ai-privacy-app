...
# Pre-check some items so the scorecard doesn't start at 0%.
# Each dimension has 10 controls, so:
# - GOVERN 30%  -> 3 checked
# - MAP 60%     -> 6 checked
# - MEASURE 20% -> 2 checked
# - MANAGE 50%  -> 5 checked
DEFAULT_CHECKED_INDICES: dict[str, set[int]] = {
    "GOVERN": {0, 3, 7},
    "MAP": {0, 1, 3, 5, 7, 9},
    "MEASURE": {1, 6},
    "MANAGE": {0, 2, 4, 7, 9},
    ...
    for dim in DIM_ORDER:
        for i in range(len(QUESTIONS[dim])):
            ...
            if k not in st.session_state:
                st.session_state[k] = i in DEFAULT_CHECKED_INDICES.get(dim, set())


def compute_scores() -> tuple[dict[str, int], dict[str, list[str]], int]:
    scores: dict[str, int] = {}
...
def render() -> None:
    ...
    with st.sidebar:
        ...
        st.caption(
            "Answer the 40 controls (Yes/No). Each **Yes = 1 point**, **No = 0 points**."
        )
        st.markdown("---")
        ...
        use_case = st.text_input(
            ...
            placeholder="e.g., Sepsis alert model, Scheduling optimizer, LLM triage bot",
        )
        risk_tier = st.selectbox(
            "Risk tier", ["Unspecified", "Low", "Medium", "High"], index=0
        )
        st.markdown("---")
                ...
                data=str(
                    {
                        ...
                        if any(k.startswith(d) for d in DIM_ORDER)
                    }
                ),
    ...
    subtitle = (
        f"<span class='pill'>Overall Score: <b>{overall}/100</b></span>  &nbsp; "
        f"<span class='pill'>Generated: <b>{datetime.now().strftime('%Y-%m-%d %H:%M')}</b></span>"
    )
    if use_case.strip():
    ...
    for idx, dim in enumerate(DIM_ORDER):
        with cols[idx]:
            ...
            st.markdown(
                f"<div class='dim-title'>{dim}</div>", unsafe_allow_html=True
            )
            st.markdown(
                f"<div class='muted'>{dim_badge(scores[dim])}</div>",
                unsafe_allow_html=True,
            )
    ...
    st.caption(
        "These are the *highest-leverage* items to complete next (all currently scored 0)."
    )
    ...
    for dim in ranked_dims:
        ...
        if not miss:
            continue
        with st.expander(
            f"{dim} — {len(miss)} missing controls (Score {scores[dim]}/100)",
            expanded=(dim == ranked_dims[0]),
        ):
    ...
    if all(len(missing[d]) == 0 for d in DIM_ORDER):
        st.success(
            "Nice — all controls are marked complete. Your Readiness Score is 100/100."
        )
    ...
    summary_lines.append(
        f"- Your strongest area is **{highest}** at **{scores[highest]}/100**."
    )
    summary_lines.append(
        f"- Your biggest opportunity is **{lowest}** at **{scores[lowest]}/100**."
    )
    if missing[lowest]:
        summary_lines.append(
            f"- Next best step: complete the top missing items in **{lowest}** to lift your overall posture quickly."
        )
    st.markdown("\n".join(summary_lines))

    st.caption(
        "Tip: Keep the sidebar as the single input surface; treat the main panel as the executive dashboard + prioritized roadmap."
    )
